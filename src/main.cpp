...
#if defined(NDEBUG)
# error "Litenowcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x37f5e2c7c54339e83bd81c26dc0229d1c840e7ad9250f31180a814d88e551d6b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Litenowcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Litenowcoin Signed Message:\n";
...
{
    // Litenowcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Litenowcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2.5 * 24 * 60 * 60; // Litenowcoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Litenowcoin: 3second
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Litenowcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Litenowcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1f922b8f4c92c47bb12c438606572b75fc7a979e51483d042088f2bd6c6014c2");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 05/Oct/2011 Steve Jobs, Apple’s Visionary, Dies at 55";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044f8f09c0919b0a5dbf1d3c2492b4c0033d5e7d748434a3911d6274f4161d0ac569e8bd238f5bbd57f34eee7455314c953a08cbaec50d3597ca2f663de2aecfe6") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1534744670;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084955418;

        if (fTestNet)
        {
            block.nTime    = 1534744636;
            block.nNonce   = 386880086;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xdcd0a7ba90f6c1b02f9e3be03cb5b4bab2a38a3ab493ecab542fe20a81013f99"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc3, 0xb7, 0xd4 }; // Litenowcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LitenowcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LitenowcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LitenowcoinMiner(CWallet *pwallet)
{
    printf("LitenowcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("litenowcoin-miner");
    ...
    try { loop {
        ...
        printf("Running LitenowcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LitenowcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LitenowcoinMiner, pwallet));
}
